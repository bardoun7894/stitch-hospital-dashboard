...
<div class="max-w-2xl mx-auto">
    ...
    <div class="flex items-center gap-4 mb-6">
        ...
        <div class="flex items-center gap-3">
            ...
            <div>
                <h1 class="text-2xl font-bold text-[#111418] dark:text-white">{{ __('messages.edit_coupon') }}</h1>
                <p class="text-sm text-[#637388] dark:text-[#9ca3af] font-mono">{{ $coupon['code'] ?? '' }}</p>
            </div>
        </div>
    </div>
    ...
    {{-- Usage Stats --}}
    <div class="mb-6 bg-white dark:bg-[#1a222e] rounded-xl border border-[#e5e7eb] dark:border-[#2d3748] p-4 flex items-center gap-4">
        ...
        <div>
            <p class="text-sm text-[#637388] dark:text-[#9ca3af]">{{ __('messages.current_uses') }}</p>
            <p class="text-lg font-bold text-[#111418] dark:text-white">
                {{ $coupon['current_uses'] ?? 0 }} / {{ ($coupon['max_uses'] ?? 0) > 0 ? $coupon['max_uses'] : __('messages.unlimited') }}
            </p>
        </div>
    </div>

    {{-- Form --}}
    <form action="{{ route('coupons.update', $coupon['id']) }}" method="POST" class="bg-white dark:bg-[#1a222e] rounded-xl border border-[#e5e7eb] dark:border-[#2d3748] p-6 space-y-5" x-data="{
        ...
        <div>
            ...
            <div class="flex gap-2">
                <input type="text" name="code" x-model="code" value="{{ old('code', $coupon['code'] ?? '') }}" required
        ...
        <div class="grid grid-cols-2 gap-4">
            <div>
                ...
                <input type="date" name="valid_from" value="{{ old('valid_from', $coupon['valid_from'] ?? '') }}" required
                    class="w-full px-4 py-2.5 border border-[#e5e7eb] dark:border-[#2d3748] rounded-lg bg-white dark:bg-[#222b3a] text-sm focus:ring-2 focus:ring-primary/30 focus:border-primary">
            </div>
            <div>
                <label class="block text-sm font-medium text-[#111418] dark:text-white mb-1.5">{{ __('messages.valid_to') }} *</label>
                <input type="date" name="valid_to" value="{{ old('valid_to', $coupon['valid_to'] ?? '') }}" required
                    class="w-full px-4 py-2.5 border border-[#e5e7eb] dark:border-[#2d3748] rounded-lg bg-white dark:bg-[#222b3a] text-sm focus:ring-2 focus:ring-primary/30 focus:border-primary">
            </div>
        </div>
        ...
        <div>
            ...
            <input type="number" name="max_uses" value="{{ old('max_uses', $coupon['max_uses'] ?? 0) }}" required min="0"
                class="w-full px-4 py-2.5 border border-[#e5e7eb] dark:border-[#2d3748] rounded-lg bg-white dark:bg-[#222b3a] text-sm focus:ring-2 focus:ring-primary/30 focus:border-primary"
                placeholder="0">
        </div>
        ...
        <div>
            ...
            <select name="clinic_id"
                ...
                <option value="{{ $clinic['id'] }}" {{ old('clinic_id', $coupon['clinic_id'] ?? '') === $clinic['id'] ? 'selected' : '' }}>
        ...
        <div class="flex items-center gap-3">
            <label class="relative inline-flex items-center cursor-pointer">
                <input type="checkbox" name="is_active" value="1" {{ old('is_active', $coupon['is_active'] ?? false) ? 'checked' : '' }} class="sr-only peer">
