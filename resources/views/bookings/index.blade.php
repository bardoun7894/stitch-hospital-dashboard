...
<main class="flex-1 flex flex-col h-full overflow-hidden bg-background-light dark:bg-background-dark relative">
    ...
    <!-- TopNavBar (Specific to this view, overrides layout header if needed, but we keep consistent layout) -->
    <!-- Note: In standard layout we use the sidebar. This view has a specific header in design. -->
    <div class="flex items-center justify-between whitespace-nowrap border-b border-solid border-b-[#f0f2f4] dark:border-[#2d3748] px-6 py-4 bg-white dark:bg-[#1a2027] shrink-0 mb-6 rounded-xl shadow-sm">
         <div class="flex items-center gap-4">
            ...
            <p class="hidden md:block text-[#637388] dark:text-gray-400 text-sm font-medium" id="current-date">{{ date('M d, Y • h:i A') }}</p>
        </div>
        <div class="flex items-center gap-4">
            <div class="relative hidden md:flex items-center">
                <span class="material-symbols-outlined absolute left-3 text-[#637388]">search</span>
                <input class="h-10 w-64 rounded-full border-none bg-[#f0f2f4] dark:bg-[#2d3748] dark:text-white pl-10 pr-4 text-sm focus:ring-2 focus:ring-primary/50" placeholder="{{ __('messages.search_patient_token') }}"/>
            </div>
            ...
            <div class="xl:col-span-4 flex flex-col gap-6">
                ...
                <div class="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-1 gap-4">
                    ...
                    <div class="bg-white dark:bg-[#1a2027] p-5 rounded-xl border border-[#e5e7eb] dark:border-[#2d3748] shadow-sm flex items-center justify-between">
                        <div>
                            ...
                            <h3 class="text-[#111418] dark:text-white text-2xl font-bold">{{ $data['stats']['bookings_today'] }}</h3>
                        </div>
                    ...
                    <div class="bg-white dark:bg-[#1a2027] p-5 rounded-xl border border-[#e5e7eb] dark:border-[#2d3748] shadow-sm flex items-center justify-between">
                        <div>
                            ...
                            <h3 class="text-[#111418] dark:text-white text-2xl font-bold">{{ $data['stats']['arrived'] }}</h3>
                        </div>
                    ...
                    <div class="col-span-2 lg:col-span-1 bg-white dark:bg-[#1a2027] p-5 rounded-xl border border-[#e5e7eb] dark:border-[#2d3748] shadow-sm flex items-center justify-between">
                        <div>
                            <p class="text-[#637388] dark:text-gray-400 text-sm font-medium mb-1">{{ __('messages.avg_wait_time_stat') }}</p>
                            <h3 class="text-[#111418] dark:text-white text-2xl font-bold">{{ $data['stats']['avg_wait'] }}</h3>
                        </div>
                        <div class="bg-orange-50 dark:bg-orange-900/20 text-orange-700 dark:text-orange-400 px-2 py-1 rounded text-xs font-medium flex items-center gap-1">
                            <span class="material-symbols-outlined text-xs">trending_down</span>
                            {{ $data['stats']['wait_trend'] }}
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white dark:bg-[#1a2027] rounded-xl border border-[#e5e7eb] dark:border-[#2d3748] shadow-md flex flex-col overflow-hidden">
                    ...
                    <div class="p-6 flex flex-col items-center text-center border-b border-[#e5e7eb] dark:border-[#2d3748]">
                        ...
                        <h1 class="text-primary text-6xl font-black tracking-tight mb-2">{{ $data['current_serving']['token'] }}</h1>
                        <p class="text-[#111418] dark:text-white text-xl font-semibold">{{ $data['current_serving']['patient'] }}</p>
                        <p class="text-[#637388] dark:text-gray-400 text-sm">{{ $data['current_serving']['type'] }}</p>
                        @if($data['current_serving']['token'] !== '--' && !empty($data['current_serving']['id']))
                    ...
                    <div class="p-6 bg-[#f8fafc] dark:bg-[#1e2530]">
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            ...
                            <button onclick="togglePauseQueue()" id="pauseButton" class="bg-white dark:bg-[#2d3748] border border-[#dce0e5] dark:border-[#4a5568] text-[#111418] dark:text-white hover:bg-gray-50 dark:hover:bg-[#384455] font-semibold py-2 px-3 rounded-lg text-sm flex items-center justify-center gap-2 transition-colors">
                    ...
                    <div class="p-5 border-b border-[#e5e7eb] dark:border-[#2d3748]">
                        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
                            ...
                            <div class="flex gap-2">
                                <button class="p-2 text-[#637388] hover:text-[#111418] dark:hover:text-white transition-colors">
                         ...
                         <div class="flex gap-6 overflow-x-auto pb-1 no-scrollbar border-b border-[#f0f2f4] dark:border-[#2d3748]">
                            <button class="pb-3 border-b-2 border-primary text-primary font-semibold text-sm whitespace-nowrap">All Bookings</button>
                            <button class="pb-3 border-b-2 border-transparent text-[#637388] dark:text-gray-400 hover:text-[#111418] dark:hover:text-white font-medium text-sm whitespace-nowrap transition-colors">Checked In (4)</button>
                            <button class="pb-3 border-b-2 border-transparent text-[#637388] dark:text-gray-400 hover:text-[#111418] dark:hover:text-white font-medium text-sm whitespace-nowrap transition-colors">Pending (12)</button>
                             <button class="pb-3 border-b-2 border-transparent text-[#637388] dark:text-gray-400 hover:text-[#111418] dark:hover:text-white font-medium text-sm whitespace-nowrap transition-colors">Completed (24)</button>
                        </div>
                    </div>
                        ...
                        <table class="w-full text-left border-collapse">
                            <thead>
                             ...
                             <tbody class="divide-y divide-[#e5e7eb] dark:divide-[#2d3748]">
                                ...
                                @php
                                    $rowClass = $booking['status'] == 'In Progress' ? 'bg-primary/5 dark:bg-primary/5' : 'hover:bg-[#f8fafc] dark:hover:bg-[#2d3748]';
                                    $statusColor = match($booking['color']) {
                                        'blue' => 'blue',
                                        'green' => 'green',
                                        'yellow' => 'yellow',
                                        'red' => 'red',
                                ...
                                <tr class="{{ $rowClass }} transition-colors">
                                    <td class="py-4 px-6">
                                        <span class="inline-flex items-center justify-center px-2.5 py-1 rounded font-bold {{ $booking['status'] == 'In Progress' ? 'bg-primary text-white shadow-sm' : 'text-[#111418] dark:text-white bg-[#f0f2f4] dark:bg-[#2d3748]' }} text-sm">{{ $booking['token'] }}</span>
                                    </td>
                                    <td class="py-4 px-6">
                                        <div class="flex items-center gap-3">
                                            <img src="{{ $booking['avatar'] }}" class="size-8 rounded-full object-cover">
                                            <div>
                                    ...
                                    <td class="py-4 px-6">
                                         <div class="flex items-center gap-2">
                                            <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium bg-{{ $statusColor }}-100 text-{{ $statusColor }}-800 dark:bg-{{ $statusColor }}-900/50 dark:text-{{ $statusColor }}-300">
                                                <span class="size-1.5 rounded-full bg-{{ $statusColor }}-500"></span>
                                                {{ $booking['status'] }}
                                            </span>
                                            
                                            @if($booking['is_arrived'])
                                            <div class="inline-flex items-center justify-center p-1 rounded-full bg-green-100 text-green-700 dark:bg-green-900/50 dark:text-green-400" title="Patient Arrived via GPS">
                                    ...
                                    <td class="py-4 px-6 text-sm text-[#111418] dark:text-white">{{ $booking['time'] }}</td>
                                     <td class="py-4 px-6 text-right flex justify-end gap-2">
                                        ...
                                        @if($booking['status'] == 'Pending')
                                            <button onclick="updateBooking('{{ $booking['id'] }}', 'accept')" class="bg-blue-50 hover:bg-blue-100 text-blue-600 p-1.5 rounded-md transition-colors" title="Accept Booking">
                                                <span class="material-symbols-outlined text-lg">check_circle</span>
                                            </button>
                                            <button onclick="updateBooking('{{ $booking['id'] }}', 'reject')" class="bg-red-50 hover:bg-red-100 text-red-600 p-1.5 rounded-md transition-colors" title="Reject Booking">
                                                <span class="material-symbols-outlined text-lg">cancel</span>
                                            </button>
                                        @elseif($booking['status'] == 'Accepted')
                                            <button onclick="updateBooking('{{ $booking['id'] }}', 'confirm-payment')" class="bg-green-50 hover:bg-green-100 text-green-600 p-1.5 rounded-md transition-colors" title="Confirm Payment">
                                                <span class="material-symbols-outlined text-lg">payments</span>
                                            </button>
                                        @elseif($booking['status'] == 'Confirmed' && !$booking['is_arrived'])
                                            <button onclick="updateBooking('{{ $booking['id'] }}', 'mark-arrived')" class="bg-purple-50 hover:bg-purple-100 text-purple-600 p-1.5 rounded-md transition-colors" title="Mark Arrived Manually">
                                                <span class="material-symbols-outlined text-lg">flight_land</span>
                                            </button>
                                            <a href="{{ route('bookings.reschedule', $booking['id']) }}" class="bg-orange-50 hover:bg-orange-100 text-orange-600 p-1.5 rounded-md transition-colors" title="Reschedule">
                                                <span class="material-symbols-outlined text-lg">calendar_month</span>
                                            </a>
                                            <button onclick="cancelBooking('{{ $booking['id'] }}')" class="bg-red-50 hover:bg-red-100 text-red-600 p-1.5 rounded-md transition-colors" title="Cancel Booking">
                                                <span class="material-symbols-outlined text-lg">block</span>
                                            </button>
                                        @endif
                                    </td>
...
let queueState = {
    clinicId: '{{ $data['queue_state']['clinic_id'] ?? 'cardiology_center' }}',
    doctorId: '{{ $data['queue_state']['doctor_id'] ?? 'doc_cardiology_center' }}',
    ...
    try {
        const response = await fetch('/queue/skip', {
            ...
            body: JSON.stringify({
                ...
                doctor_id: queueState.doctorId,
                date: queueState.date,
                token_number: getCurrentToken(), // Get from UI
...
async function togglePauseQueue() {
    ...
    try {
        ...
        if (data.success) {
            queueState.isPaused = newPausedState;
        ...
        showNotification('An error occurred while pausing/resuming queue', 'error');
    }
}
...
function updatePauseButton(isPaused) {
    ...
    if (isPaused) {
        ...
        button.classList.remove('bg-white', 'text-[#111418]', 'border-[#dce0e5]');
    } else {
        icon.textContent = 'pause';
        if (text) text.textContent = 'Pause Queue';
        button.classList.remove('bg-green-50', 'text-green-600', 'border-green-200');
        button.classList.add('bg-white', 'text-[#111418]', 'border-[#dce0e5]');
    }
}

function getCurrentToken() {
    // Extract current token from UI
    const tokenElement = document.querySelector('h1.text-primary');
    return tokenElement ? parseInt(tokenElement.textContent.replace(/\D/g, '')) : 0;
}
